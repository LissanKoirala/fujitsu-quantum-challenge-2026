...
USE_SPLIT = True  # default: split mode (laptop-safe), override with --full
...
def test_2_circuit_init():
    ...
    cfg = QubitConfig30Q()
    split = USE_SPLIT
    print(f"  creating HybridQGANCircuit30Q (n_layers=2, split={split})...", flush=True)
    circuit = HybridQGANCircuit30Q(n_layers=2, config=cfg, split_registers=split)
...
def test_5_generator_init():
    ...
    split = USE_SPLIT
    print(f"  creating QGAN_Generator30Q (split={split})...", flush=True)
    gen = QGAN_Generator30Q(noise_dim=32, num_linker_atoms=10, n_circuit_layers=2,
                            split_registers=split)
