...
class HybridReadoutMapper30Q(nn.Module):
    ...
    def forward(
        ...
        # Project to linker + lattice space
        linker_lattice_raw = self.latent_head(latent_features)  # (batch, 3*num_linker + 6)
        ...
        # Detach outputs to allow numpy conversion while keeping computation graph for training
        return {
            'metal_atoms': metal_atoms.detach(),
            'linker_atoms': linker_atoms.detach(),
            'lattice_params': lattice_params.detach(),
